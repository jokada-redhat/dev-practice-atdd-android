...
class ReturnBookUseCase(
    ...
    private companion object {
        const val TAG = "ReturnBookUseCase"
    }
    fun execute(request: ReturnBookRequest): ReturnBookResult {
        ...
        val updateMemberResult = memberRepository.updateLoanCount(request.memberId, newLoanCount)
        if (updateMemberResult.isFailure) {
            // ロールバック
            loanRepository.save(activeLoan)
            val rollback = bookRepository.updateStatus(request.bookId, BookStatus.BORROWED)
